...
def main():
        ...
        with st.expander("⚗️ Transform / Lifting", expanded=True):
            ...
            if not available_for_domain:
                ...
                selected_lifting_name = st.selectbox(
                    ...
                    help=f"Choose a {selected_domain} → {selected_target} lifting"
                )
                ...
                with st.expander("📄 Config preview", expanded=False):
                    cfg_display = {
                        k: v for k, v in selected_lifting['config'].items()
                        if k not in ('neighborhoods',)  # skip verbose interpolated fields
                    }
                    st.json(cfg_display)
        ...
        if st.button("🔄 Load Dataset", type="primary", use_container_width=True):
            with st.spinner("Loading dataset..."):
                try:
                    ...
                    st.session_state['data_original'] = data   # keep a clean copy for reset
                    st.session_state['data_domain'] = loaded_domain
                    st.session_state['neighborhoods'] = all_neighborhoods
                    st.session_state['lifting_applied'] = None  # clear any previous lifting
                    st.success(f"✅ {selected_dataset} ({loaded_domain}) loaded!")
                except Exception as e:
                    st.error(f"Error loading dataset: {str(e)}")

        # Apply Lifting / Reset — only shown once a dataset is loaded
        if 'data' in st.session_state:
            selected_lifting = st.session_state.get('selected_lifting')
            lifting_applied = st.session_state.get('lifting_applied')

            if selected_lifting:
                col_apply, col_reset = st.columns(2)
                with col_apply:
                    if st.button("⚗️ Apply Lifting", use_container_width=True,
                                 disabled=(lifting_applied is not None and
                                           lifting_applied['name'] == selected_lifting['name'])):
                        with st.spinner(f"Applying {selected_lifting['name']}..."):
                            # Placeholder — actual transform call goes here
                            st.session_state['lifting_applied'] = selected_lifting
                            st.success(f"✅ Lifting applied!")
                with col_reset:
                    if st.button("↩️ Reset", use_container_width=True,
                                 disabled=(lifting_applied is None)):
                        st.session_state['data'] = st.session_state['data_original']
                        st.session_state['lifting_applied'] = None
    ...
    if hasattr(data, 'num_nodes'):
        ...
        if lifting_applied:
            ...
            st.info(f"✅ Dataset loaded! Nodes: {num_nodes}, Node features: {num_features if num_features else 'N/A'}")
    else:
    ...
    if not has_neighborhoods:
        ...
        if lifting_applied:
            st.success(
                f"⚗️ **Lifting applied:** `{lifting_applied['name']}` "
                f"({lifting_applied['source']} → {lifting_applied['target']}). "
                f"Use ↩️ Reset in the sidebar to go back to the original data."
            )
        else:
            selected_lifting = st.session_state.get('selected_lifting')
            if selected_lifting:
                st.info(
                    f"⚗️ **{selected_lifting['name']}** selected "
                    f"({selected_lifting['source']} → {selected_lifting['target']}). "
                    f"Click **Apply Lifting** in the sidebar to apply it."
                )
            else:
